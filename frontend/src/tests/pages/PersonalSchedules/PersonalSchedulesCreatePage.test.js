...
describe("PersonalSchedulesCreatePage tests", () => {
    ...
    test("when you fill in the form and hit submit with a name with length greater than fifteen, the name on the toast is shortened", async () => {
        const queryClient = new QueryClient();
        const personalSchedule = {
            id: 17,
            name: "SampNameLongerThanFifteenChars",
            description: "desc",
            quarter: "W08"
        };
        ...
        expect(mockToast).toBeCalledWith("New personalSchedule Created - id: 17 name: SampNameLongerT");
        expect(mockNavigate).toBeCalledWith({ "to": "/personalschedules/list" });
    });
